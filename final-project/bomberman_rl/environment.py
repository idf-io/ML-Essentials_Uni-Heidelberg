...
prev_bombs = deque(maxlen=2)  # First entry = empty list to avoid first round problem
prev_bombs.append([])
...
class BombeRLeWorld(GenericWorld):
    ...
    def send_game_events(self):
        ...
        for a in self.agents:
            ...
            '''
            '''
            new_game_state=self.get_state_for_agent(a)
            prev_bombs.append([bomb for bomb in a.last_game_state['bombs'] if bomb[1] == 0])
            #print(a.last_game_state)
            old_game_state = a.last_game_state
            #print(old_game_state.size)

            old_state = state_to_features( old_game_state,prev_bombs[0])
            new_state = state_to_features( new_game_state,old_game_state['bombs'])


            action = a.last_action
            #new_game_state = state_to_features(self.get_state_for_agent(a),prev_bombs[0])[0]
            if( new_state is None): pass
            else:new_state=str(new_state)
            reward = reward_from_events(a.events)
            #f.writelines(" ".join(str(i) for i in every_state))
            tttt=time()
            if(len(old_state)==25 and new_state is not None):
                datasets[a.name][tttt]=Transition(str(old_state), action, new_state,reward)
            ##############
            '''
            if a.train:
                if not a.dead:
...
class GUI:
    ...
    def render(self):
        ...
        if not self.world.running:
            ...
            global save_flag
            if(save_flag==1):
                with open('datasets_allfeature_10000.json', 'a', encoding='utf8') as fp:
